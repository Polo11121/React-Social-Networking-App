...
export const useFilters = (afterUpdate: () => void) => {
  ...
  const changeInterestedGendersHandler = (
    value: SingleValue<SelectOptionType>
  ) => formik.setFieldValue('interestedGenders', value);

  const changeInterestedAgeHandler = (value: SingleValue<SelectOptionType>) =>
    formik.setFieldValue('interestedAge', value);

  const changeInterestedCityHandler = (value: SingleValue<SelectOptionType>) =>
    formik.setFieldValue('interestedCity', value);

  const changeInterestedCityMaxDistanceHandler = (
    value: SingleValue<SelectOptionType>
